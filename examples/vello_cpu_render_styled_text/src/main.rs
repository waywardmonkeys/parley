...
fn main() {
    ...
    styled.push_with(
        "BIG",
        ParleyStyleChange::new()
            .with(StyleProperty::FontSize(32.0))
            .with(StyleProperty::FontWeight(FontWeight::BOLD))
            .with(StyleProperty::LetterSpacing(1.0))
            .with(StyleProperty::Brush(red)),
    );
    styled.push(", ");
    let tiny = styled.push_with(
        "tiny",
        ParleyStyleChange::from(StyleProperty::FontSize(12.0)),
    );
    styled.apply(tiny, ParleyStyleChange::from(StyleProperty::Brush(purple)));
    styled.push(", ");
    styled.push_with(
        "underline",
        ParleyStyleChange::new()
            .with(StyleProperty::Underline(true))
            .with(StyleProperty::Brush(green)),
    );
    styled.push(", ");
    styled.push_with(
        "strike",
        ParleyStyleChange::new()
            .with(StyleProperty::Strikethrough(true))
            .with(StyleProperty::Brush(purple)),
    );
    styled.push(", and ");
    styled.push_with(
        "paint-only color",
        ParleyStyleChange::from(StyleProperty::Brush(red)),
    );
    styled.push(".\n");
